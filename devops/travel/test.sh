...
travel --debug config add python 2.7.1 $( python -c 'import sys; print(sys.executable)' )
( travel setup && exit 1 ) || echo Wrong python version ok

# right config
python -c 'import sys; print("python: " + sys.version.split(" ")[0])' > bag.yml
travel --debug config add python $( python -c 'import sys; print(sys.version.split()[0], sys.executable)' )
travel --debug setup
# End Python version
